...
**HTTPS（Hypertext Transfer Protocol Secure）**は、HTTPをTLSで保護した通信です。式で表せば、**HTTPS = HTTP over TLS**です。HTTPSという独立した新しい要求・応答方式があるのではなく、TLSで安全な通信路を構築してから、その中でHTTPをやり取りします。通常はTCP 443番を使います。HTTP/3ではQUICとTLS 1.3をUDP上で統合しますが、DNSで接続先を知り、相手を認証し、HTTP要求を送る目的は同じです。

# 5. 詳しい仕組み

## URL入力から表示まで

基本の順序は次のとおりです。HTTP Requestを送るのは、TLSハンドシェイクと証明書検証が完了した後です。
...
この流れを通信相手とのやり取りとして表すと、次のようになります。
